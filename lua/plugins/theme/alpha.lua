...
local ANIME = {
    {"02.txt", 54, 26}, {"Lucy.txt", 46, 21}, {"Ayanami.txt", 59, 32}
    -- { "Asuka.txt", 167, 34 },
    -- { "02_gif.txt", 84, 26 },
    -- { "02_1_gif.txt", 81, 25 },
    -- { "02_2_gif.txt", 81, 25 },
...
return {
                ...
                maps.n["<Leader>h"] = {
                    function()
                        local is_valid_win = function(win)
                            ...
                            return
                                require("astrocore.buffer").is_valid(bufnr) or
                                    vim.bo[bufnr].filetype == "alpha"
                        end
                        local cur_win = vim.api.nvim_get_current_win()
                        -- try to find the most suitable window for Alpha
                        if not is_valid_win(cur_win) then
                            for _, win in ipairs(
                                              vim.api.nvim_tabpage_list_wins(0)) do
                ...
                opts.autocmds.alpha_settings = {
                    {
                        ...
                        callback = function(event)
                            if ((event.event == "User" and event.file ==
                                "AlphaReady") or
                                (event.event == "BufWinEnter" and
                                    vim.bo[event.buf].filetype == "alpha")) and
                                not vim.g.before_alpha then
                                vim.g.before_alpha = {
                                    showtabline = vim.opt.showtabline:get(),
                                    laststatus = vim.opt.laststatus:get(),
                                    cmdheight = vim.opt.cmdheight:get()
                                }
                                vim.opt.showtabline, vim.opt.laststatus, vim.opt
                                    .cmdheight = 0, 0, 0
                            elseif vim.g.before_alpha and event.event ==
                                "BufWinEnter" and vim.bo[event.buf].buftype ~=
                                "nofile" then
                                vim.opt.laststatus, vim.opt.showtabline, vim.opt
                                    .cmdheight = vim.g.before_alpha.laststatus,
                ...
                opts.autocmds.alpha_autostart = {
                    {
                        ...
                        callback = function()
                            ...
                            if vim.fn.argc() > 0 -- don't start when opening a file
                            ...
                            then
                                ...
                                for _, arg in pairs(vim.v.argv) do
                                    if arg == "-b" or arg == "-c" or
                                        vim.startswith(arg, "+") or arg == "-S" then
                                        should_skip = true
                                        break
                                    end
                                end
    ...
    opts = function()
        ...
        dashboard.button = function(shortcut, desc, rhs, map_opts)
            ...
            local lhs = shortcut:gsub("%s", ""):gsub(dashboard.leader,
                                                     "<Leader>")
            ...
            return {
                ...
                on_press = function()
                    vim.api.nvim_feedkeys(
                        vim.api.nvim_replace_termcodes(rhs or lhs .. "<Ignore>",
                                                       true, false, true), "t",
                        false)
                end,
                opts = {
                    ...
                    keymap = rhs and
                        {
                            "n", lhs, rhs,
                            require("astrocore").extend_tbl(default_map_opts,
                                                            map_opts)
                        }
                }
            }
        ...
        dashboard.section.buttons.val = {
            dashboard.button("LDR n  ",
                             get_icon("FileNew", 2, true) .. "New File  "),
            dashboard.button("LDR f f",
                             get_icon("Search", 2, true) .. "Find File  "),
            dashboard.button("LDR f o",
                             get_icon("DefaultFile", 2, true) .. "Recents  "),
            dashboard.button("LDR f w",
                             get_icon("WordFile", 2, true) .. "Find Word  "),
            dashboard.button("LDR f '",
                             get_icon("Bookmarks", 2, true) .. "Bookmarks  "),
            dashboard.button("LDR S l",
        ...
        if vim.loop.os_uname().sysname == "Linux" then
            ...
            local path = os.getenv "HOME" .. "/.config/nvim/sources/"

            if idx > 3 then
                dashboard.section.terminal.command =
                    "sh " .. path .. "show.sh " .. path .. info[1]
            else
            ...
            dashboard.opts.layout = {
                dashboard.section.terminal, {type = "padding", val = 2},
        ...
        vim.api.nvim_create_autocmd("User", {
            ...
            callback = function()
                ...
                opts.section.footer.val = {
                    "AstroNvim loaded " .. stats.loaded .. "/" .. stats.count ..
                        " plugins " ..
                        require("astroui").get_icon("Package", 1, true) .. "in " ..
                        ms .. "ms"
                }
